...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) {
        User user = userRepository.findByUsername(username) // 저장소에서 유저 이름을 받아오기
                .orElseThrow(() -> new UsernameNotFoundException(username));

        // Spring Security의 ROLE_ 접두사 컨벤션
        SimpleGrantedAuthority authority = new SimpleGrantedAuthority("ROLE_" + user.getRole()); // 정해진것. 롤을 이렇게 넣어주기 -> 객체에 권한이 들어간다.
        ...
        String password = user.getPasswordHash() != null ? user.getPasswordHash() : ""; // OAUTH에서는 비밀번호 개념이 없어서 이렇게 만듦.

        return new org.springframework.security.core.userdetails.User(
                user.getUsername(),
                password,
                List.of(authority) // 유저 객체를 만들어서 떨궈준다. 이렇게 하면 contextholder 로 들어간다.
        );
    }
}
