...
import com.sesac.aibackend.sp03_security_rag.dto.ChatRequest;
import com.sesac.aibackend.sp03_security_rag.dto.ChatResponse;
import com.sesac.aibackend.sp03_security_rag.service.PythonChatClient;
...
public class ChatController {
    ...
    public ChatResponse chat(
        ...
        String username = user.getUsername(); // @AuthenticationPrincipal 인증 객체에 user가 없기 때문에 username으로 변경함
        ChatResponse response = pythonClient.chat(req.prompt()).block(); // fastapi로 보내서 결과값을 받는곳이 여기다.
        if (response == null) {
            throw new IllegalStateException("python returned null");
        }

        chatLogService.save(username, req.prompt(), response.answer()); // 저장때 사용
